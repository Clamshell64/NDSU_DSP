...
// Set 12-bit raw DAC value (0..4095). Clips values greater than 4095.
void DAC_SetRaw(uint16_t value){
    if (value > 4095u) value = 4095u;
    uint8_t low = (uint8_t)(value & 0xFFu);
    uint8_t high = (uint8_t)((value >> 8) & 0x0Fu);
    DAC0->DAT[0].DATL = DAC_DATL_DATA0(low);
    DAC0->DAT[0].DATH = DAC_DATH_DATA1(high);
    DAC0->C0 |= DAC_C0_DACSWTRG_MASK; // issue software trigger to update output
}

void DAC_SetVoltage(float volts){
	if(volts < 0.0f) volts = 0.0f;
	if(volts > 3.0f) volts = 3.0f;
	uint32_t val = (uint32_t)((volts / 3.0f) * 4095.0f);
	DAC_SetRaw((uint16_t)val);
}
